...
public class JPNotice extends JFrame {
	...
	public JPNotice() {
		...
		try {
			File f = new File(Variable.Subject + "의 공지사항]titles.txt");
			if (f.exists()) {

				read1 = new FileReader(Variable.Subject + "의 공지사항]titles.txt");
	...
	public void actionfind() {
		find.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				Variable.Title = Variable.Subject + "의 공지사항]" + number + ".txt";
				File f = new File(Variable.Subject + "의 공지사항]" + number
						+ ".txt");
				if (!f.exists()) {
					JOptionPane.showMessageDialog(null, "없는 번호입니다!");
				} else {

					JPNoticeR br = new JPNoticeR();
					br.setVisible(true);
					dispose();
